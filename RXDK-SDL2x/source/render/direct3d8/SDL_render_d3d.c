...
D3D_CreateRenderer(SDL_Window* window, Uint32 flags)
{
	...
	renderer->QueueSetDrawColor = D3D_QueueSetViewport;  /* no-op in this backend */
	renderer->QueueDrawPoints = D3D_QueueDrawPoints;
	renderer->QueueDrawLines = D3D_QueueDrawPoints;   /* lines and points share queuing */
	...
	/* Base driver info; you already set it from D3D_RenderDriver. */
	renderer->info = D3D_RenderDriver.info;
	/* We know we’re accelerated and support target textures here. */
	renderer->info.flags = (SDL_RENDERER_ACCELERATED | SDL_RENDERER_TARGETTEXTURE);

	renderer->driverdata = data;
	renderer->window = window;                 /* IMPORTANT: used elsewhere */
	...
	/* Validate requested size against dashboard flags; fallback if unsupported. */
	{
		vidflags = XGetVideoFlags();
		const SDL_bool is_pal = (XGetVideoStandard() == XC_VIDEO_STANDARD_PAL_I) ? SDL_TRUE : SDL_FALSE;

		SDL_bool supported = SDL_FALSE;
		if ((pparams.BackBufferWidth == 1920 && pparams.BackBufferHeight == 1080 && (vidflags & XC_VIDEO_FLAGS_HDTV_1080i)) ||
			(pparams.BackBufferWidth == 1280 && pparams.BackBufferHeight == 720 && (vidflags & XC_VIDEO_FLAGS_HDTV_720p)) ||
			(pparams.BackBufferWidth == 720 && pparams.BackBufferHeight == 480 && (vidflags & XC_VIDEO_FLAGS_HDTV_480p)) ||
			(pparams.BackBufferWidth == 720 && pparams.BackBufferHeight == 576  /* PAL 576i */) ||
		...
		if (!supported) {
			...
			/* Highest enabled fallback: 1080i -> 720p -> 480p -> 576i (PAL) -> 480i */
			...
			SDL_Log("Xbox D3D: requested %ux%u not permitted; falling back to %ux%u\n",
				(unsigned)ow, (unsigned)oh,
				(unsigned)pparams.BackBufferWidth, (unsigned)pparams.BackBufferHeight);
		}
	}

	/* Set timing/aspect flags to match final backbuffer size + dashboard prefs. */

	/* Clear any previous choices first. */
	pparams.Flags &= ~(D3DPRESENTFLAG_WIDESCREEN |
		D3DPRESENTFLAG_PROGRESSIVE |
		D3DPRESENTFLAG_INTERLACED);

	/* Progressive vs interlaced: 1080i and 576i are interlaced; 720p/480p progressive. */
	if (pparams.BackBufferHeight >= 1080) {
		pparams.Flags |= D3DPRESENTFLAG_INTERLACED;
	}
	else if (pparams.BackBufferHeight == 576) {
		pparams.Flags |= D3DPRESENTFLAG_INTERLACED;
	}
	else {
	...
	/* Widescreen / letterbox behavior. */
	{
		const float aspect = (float)pparams.BackBufferWidth / (float)pparams.BackBufferHeight;
		const SDL_bool is16x9 = (aspect > 1.70f && aspect < 1.80f); /* 16:9 */
		if (is16x9 && (vidflags & XC_VIDEO_FLAGS_WIDESCREEN)) {
			pparams.Flags |= D3DPRESENTFLAG_WIDESCREEN;
		}
	}

	/* Refresh rate: PAL 576 uses 50 or 60 (PAL60); others 60. */
	if (XGetVideoStandard() == XC_VIDEO_STANDARD_PAL_I &&
		pparams.BackBufferHeight == 576) {
		pparams.FullScreen_RefreshRateInHz =
			(vidflags & XC_VIDEO_FLAGS_PAL_60Hz) ? 60 : 50;
	}
	else {
